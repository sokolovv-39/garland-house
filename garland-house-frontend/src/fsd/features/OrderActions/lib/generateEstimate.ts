...
export async function generateEstimate(idb: IndexedDB, orderId: IDBValidKey) {
  ...
  objItemsArr.forEach((el) => {
    ...
    const relaysSwitches = getRelaysSwitches(el.items);
    const vagi = getEsVagi(el.items);
  ...
  aEl.href = pdfDataUri;
  aEl.download = `Смета_${order.customer}_${order.customerPhone}`;
  aEl.click();
