...
export function switchesRfp(
  ...
  if (timeRelay) {
    rfp.push({
      id: id.toString(),
      desc: "Монтаж реле времени (настраиваемый таймер вкл/выкл)",
      quantity: timeRelay.toString(),
      unit: "шт",
      price: relaysSwitchesDefault.priceObj.timeRelay.toString(),
      cost: (relaysSwitchesDefault.priceObj.timeRelay * timeRelay).toString(),
    });
    id++;
  }
