...
export function montageRfp(
  ...
  if (distance) {
    const minCost = 1500;
    const pureCost = montageDefault.priceObj.distance * distance;
    const isLessBoundary = pureCost < minCost;
    const cost = isLessBoundary ? minCost : pureCost;
    ...
    rfp.push({
      ...
      price: isLessBoundary
        ? (minCost / distance).toString()
        : montageDefault.priceObj.distance.toString(),
      cost: cost.toString(),
    });
    id++;
  }
